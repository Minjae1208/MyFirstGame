...
bool IOCPBase::Init()
{
	...
	if (nResult != 0)
	{
		printf_s("[ERROR][LoginServer] Winsock 초기화 실패 \n");
		return false;
	}
	...
	{
		printf("[ERROR][LoginServer] 소켓 생성 실패\n");
		return false;
	}
	...
	{
		printf_s("[ERROR][LoginServer] bind 실패 \n");
	...
	{
		printf_s("[ERROR][LoginServer] listen 실패\n");
		closesocket(ListenSocket);
		WSACleanup();
		return false;
	}
...
void IOCPBase::StartLoginServer()
{
	...
	while (bAccept)
	{
		...
		{
			printf_s("[ERROR][LoginServer] Accept 실패 \n");
			return;
		}
		...
		{
			printf_s("[ERROR][LoginServer] IO Pending 실패 : %d", WSAGetLastError());
			return;
		}
	}
}
...
void IOCPBase::Recv(stSocketInfo* pSocket)
{
	...
	if (nResult == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING)
	{
		printf_s("[ERROR][LoginServer] WSARecv 실패 : %d", WSAGetLastError());
	}
}
